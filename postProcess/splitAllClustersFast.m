...
if ops.lowmem 
	...
	[mpath, ~]     = fileparts(ops.fproc);
end

while ik<Nfilt
    if rem(ik, 100)==1
      % periodically write updates
       fprintf('%2.2f sec, found %d splits, checked %d/%d clusters, nccg %d, ntemp %d\n', toc, nsplits, ik, Nfilt, nccg, ntemp)
    end
    ik = ik+1;
...
%     ps = mean(rs(:,1));
    ...
    if nremove > .05 && min(plow,phigh)>ccsplit && min(sum(ilow), sum(~ilow))>300
       ...
       if ops.lowmem, splitPrincipalComponents(rez, Nfilt, ik); end
...
fprintf('Finished splitting. Found %d splits, checked %d/%d clusters, nccg %d, ntemp %d \n', nsplits, ik, Nfilt, nccg, ntemp)
toc
%if ops.lowmem , clear mfilePC; end
